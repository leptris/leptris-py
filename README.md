...
| RELAX NG | **core subset** | `leptris.RelaxNG(schema)` / `.from_file(path)` — compile once, `.validate(doc)` → bool with Jing-shaped `.error_log`; element/attribute/text/data/value/choice/group/interleave/repeats/ref — libleptris 1.9.115+ ([#878](https://github.com/leptris/leptris/issues/878)) |
